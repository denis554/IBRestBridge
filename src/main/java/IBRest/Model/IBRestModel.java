package IBRest.Model;

import IBRest.Constants.HmacSHA512;
...
public class IBRestModel {
    ...
    protected boolean isAuthHost;

    public IBRestModel(HttpServletRequest request) {
        this.request = request;
        remoteHost = request.getHeader("X-FORWARDED-FOR");
        signature = request.getHeader(HEADER_SIGNATURE);
        nonce = request.getHeader(HEADER_API_NONCE);
        apiKey = request.getHeader(HEADER_API_KEY);
        isAuthHost = checkAuthHost();
    }

    private boolean checkApiKey() {
        if (apiKey != null && apiKey.compareTo(API_KEY) == 0) {
            return true;
        }
        return false;
    }
...
//        System.out.println("getRequestURL" + request.getRequestURL());
    ...
    private boolean checkAuthHost() {
        if (checkSignature()) {
            return true;
        }
        boolean isExist = false;
        for (String eachHost: WHITE_HOST_LIST) {
            if (eachHost.equalsIgnoreCase(remoteHost)) {
                isExist = true;
                break;
            }
        }
    ...
    public JSONObject retForbiddenHost() {
        JSONObject jObj = new JSONObject();
        if (!StringUtils.isEmpty(request.getHeader("x-remote"))) {
            jObj.put("host", request.getHeader("x-remote"));
        } else {
            jObj.put("host", remoteHost);
        }
        jObj.put("msg", "This host is forbidden!");
        return jObj;
    }
